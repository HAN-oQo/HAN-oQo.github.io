...
# 1) subscription auth (no API key) — browser auth happens once
if [ -z "${CLAUDE_CODE_OAUTH_TOKEN:-}" ]; then
  echo "==> Claude 구독 토큰 발급 중 (브라우저 인증 1회)…"
...
if [ "$TUNNEL" = "1" ]; then
  ...
  tmux send-keys -t "$SESS" "CLAUDE_CODE_OAUTH_TOKEN='$CLAUDE_CODE_OAUTH_TOKEN' ALLOW_EDITS=0 ACCESS_TOKEN='$ACCESS_TOKEN' PORT=$PORT ALLOW_ORIGIN='https://han-oqo.github.io' $PY '$REPO_DIR/assets/ask-bot-server/server.py'" C-m
  ...
  tmux send-keys -t "$SESS" "CLAUDE_CODE_OAUTH_TOKEN='$CLAUDE_CODE_OAUTH_TOKEN' ALLOW_EDITS=1 ACCESS_TOKEN='$ACCESS_TOKEN' PORT=$PORT REPO_DIR='$REPO_DIR' $PY '$REPO_DIR/assets/ask-bot-server/server.py'" C-m
  tmux split-window -t "$SESS" -h -c "$REPO_DIR"
