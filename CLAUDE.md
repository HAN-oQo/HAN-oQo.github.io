...
## 긴 페이지는 청크로 작성 (수백 줄 HTML 생성 시 필수)

study/ 딥다이브·paper note·briefing 처럼 **대략 300줄 이상짜리 HTML** 을 만들 때는 한 번의
Write 로 파일 전체를 뱉지 말고 **여러 조각으로 나눠 쓴다.** 큰 단일 출력은 API 응답이 중간에
끊기는 `Response stalled mid-stream` 오류를 유발하고, 끊기면 진행분이 통째로 날아간다. 나눠 쓰면
매 단계 파일이 유효하고, 한 조각이 실패해도 앞의 진행분은 남는다.

방법 (sentinel 이어붙이기):

1. **Write** — `<head>`~첫 섹션까지 만들고, 닫는 태그 **바로 앞에** `<!-- MORE -->` 표식을 남긴 뒤
   `</div>` + 스크립트 + `</body></html>` 로 **일단 유효하게 닫는다.**
2. **Edit** — `<!-- MORE -->` 를 `다음 섹션 + <!-- MORE -->` 로 치환해 섹션을 이어붙인다. 마지막
   섹션에서는 표식을 지운다.
3. 한 조각은 섹션 한두 개(또는 큰 SVG 하나) 정도로 유지. index.html 카드 추가 같은 작은 편집은
   그냥 Edit 로 한다.

(새로 생긴 규칙이 아니라, 실제 stall 을 겪고 정한 안전 기본값이다.)
